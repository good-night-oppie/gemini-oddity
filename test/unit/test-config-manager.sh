...
test_xdg_directory_creation() {
    ...
    if [ -d "$XDG_CONFIG_HOME/claude-gemini-bridge" ] && \
       [ -d "$XDG_CONFIG_HOME/claude-gemini-bridge/providers" ] && \
       [ -d "$XDG_CONFIG_HOME/claude-gemini-bridge/auth" ] && \
       [ -d "$XDG_CACHE_HOME/claude-gemini-bridge" ]; then
...
test_default_config_creation() {
    ...
    local config_file="$XDG_CONFIG_HOME/claude-gemini-bridge/config.json"
...
test_secure_storage() {
    ...
    local secure_file="$XDG_CONFIG_HOME/claude-gemini-bridge/auth/secure.enc"
...
test_legacy_migration() {
    ...
    rm -rf "$HOME/.claude-gemini-bridge" "$HOME/.config"
    mkdir -p "$HOME/.claude-gemini-bridge/auth"
    
    # Create legacy config
    cat > "$HOME/.claude-gemini-bridge/config" << 'EOF'
...
EOF
    ...
    cat > "$HOME/.claude-gemini-bridge/auth/tokens.json" << 'EOF'
{
...
EOF
    ...
    local tokens_file="$XDG_CONFIG_HOME/claude-gemini-bridge/auth/tokens.json"
...
test_file_permissions() {
    ...
    local config_dir="$XDG_CONFIG_HOME/claude-gemini-bridge"
