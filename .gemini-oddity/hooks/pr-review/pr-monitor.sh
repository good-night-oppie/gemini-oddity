#!/bin/bash
# ABOUTME: Advanced PR review monitor with debate protocol for Gemini Oddity
...
request_claude_review() {
    ...
    local review_prompt="@claude Please review this PR for the Gemini Oddity.
...
### Conclusion
All critical concerns have been addressed with empirical validation. The Gemini Oddity maintains:
...
1. **Per-Project Installation**: Each project gets its own \`.gemini-oddity/\` directory with independent configuration
...
main() {
    ...
    case $command in
        ...
        *)
            echo "Gemini Oddity PR Review Monitor"
            echo ""
            echo "Usage:"
