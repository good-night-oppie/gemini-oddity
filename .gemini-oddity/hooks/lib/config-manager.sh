    ...
    cat > "$CONFIG_FILE" << 'EOF'
{
  ...
  "paths": {
    "logs": "${XDG_STATE_HOME:-$HOME/.local/state}/gemini-oddity/logs",
    "cache": "${XDG_CACHE_HOME:-$HOME/.cache}/gemini-oddity",
    "data": "${XDG_DATA_HOME:-$HOME/.local/share}/gemini-oddity"
  }
}
EOF
...
get_encryption_key() {
    local machine_id=""
    local user_salt="$USER-gemini-oddity"
